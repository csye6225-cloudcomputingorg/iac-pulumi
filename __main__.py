...
import pulumi
import base64
from pulumi_aws import rds, acm, get_availability_zones
...
def create_subnet(ec2, vpc, cidr_block, availability_zone, subnet_type):
    subnet = ec2.create_subnet(
        CidrBlock=cidr_block, VpcId=vpc.id, AvailabilityZone=availability_zone)
    subnet.create_tags(
        Tags=[{"Key": "Name", "Value": f"{subnet_type} {availability_zone}"}])
    return subnet
...
def create_route_table(ec2, vpc, subnet_type):
    ...
    route_table.create_tags(
        Tags=[{"Key": "Name", "Value": f"{subnet_type} Route Table"}])
    return route_table


def create_route_to_igw(route_table, internet_gateway):
    route_table.create_route(DestinationCidrBlock=os.getenv(
        "ig_cidr_block"), GatewayId=internet_gateway.id)
...
def create_or_get_key_pair(ec2_client):
    ...
    pulumi_key_pair = aws.ec2.KeyPair(
        "my-keypair_demo", public_key=new_key_pair["KeyMaterial"])
...
def main():
    ...
    for index, az in enumerate(availability_zones):
        ...
        public_subnet = create_subnet(
            ec2, vpc, f'10.0.{index * 2}.0/24', az, "Public")
        public_subnets.append(public_subnet)

        # Private subnet for current AZ
        private_subnet = create_subnet(
            ec2, vpc, f'10.0.{index * 2 + 1}.0/24', az, "Private")
    ...
    load_balancer = aws.lb.LoadBalancer('app-load-balancer',
                                        security_groups=[load_balancer_sg.id],
                                        subnets=[
                                            public_subnets[0].id, public_subnets[1].id, public_subnets[2].id],
    ...
    cert = acm.get_certificate(domain=os.environ.get('domain'), most_recent=True)
    print(cert.arn)
    ...
    rds_instance = create_rds_instance(
        db_security_group, db_parameter_group, private_subnets)
    ...
    instance_profile = aws.iam.InstanceProfile(
        "instanceProfile", role=role.name)
    ...
    user_data_base64 = pulumi.Output.from_input(user_data_script).apply(
    ...
    auto_scaling_group = aws.autoscaling.Group('web-asg',
                                               ...
                                               tags=[
                                                   {"key": "Name", "value": "webapp-asg", "propagate_at_launch": True}]
                                               )
    ...
    zone_id = aws.route53.get_zone(name=os.environ.get('domain')).zone_id

    dns_record = aws.route53.Record("webserver",
                                    type="A",
                                    zone_id=zone_id,
                                    name=os.environ.get('domain'),
                                    aliases=[
                                        {
    ...
    pulumi.export("key name", key_name)
    ...
    api_secret = aws.secretsmanager.Secret("mailgun-api-key")

    # Define the Secret's version (this is where the actual secret value is stored)
    email_secret_version = aws.secretsmanager.SecretVersion("mailgun-api-key-version",
                                                            secret_id=api_secret.id,
                                                            # "4dc82ba6f91f8bbf597a3aeced3ef791-30b58138-ae50c84a"
                                                            secret_string=os.environ.get(
                                                                'mailgun_api_key')
                                                            )
    ...
    lambda_role_policy = aws.iam.RolePolicy("lambdaRolePolicy",
                                            role=lambda_role.id,
                                            policy=pulumi.Output.all(dynamodb_table.arn, api_secret.arn).apply(lambda arns: f"""{{
        "Version": "2012-10-17",
        "Statement": [
    ...
    lambda_function = aws.lambda_.Function('my-lambda-function',
                                           ...
                                           environment=aws.lambda_.FunctionEnvironmentArgs(
                                               variables={
                                                   ...
                                                   'MAILGUN_MESSAGE_URL': os.getenv('mailgun_message_url'),
                                                   'MAILGUN_STATUS_URL': os.getenv('mailgun_status_url'),
